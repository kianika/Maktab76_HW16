...
handleAdd = async (event) => {
 ...
 const id = (this.state.id)++
  this.setState({items : [...this.state.items, {id, name, familyname, phonenumber, options, email}]});
...
render() {
  const {name, familyname, phonenumber, email, nameError, familynameError, phonenumberError, emailError} = this.state;
  const isValid = nameError === '' && familynameError === '' && phonenumberError === '' && emailError === '';
  return (
    <>
    ...
    <form>
      ...
      <div style={style}><input name="name" placeholder="name" value={name} onChange={this.handleChange}/><span className="error-message">*</span></div>
      <div className='error-message'>{nameError}</div>
      <div style={style}><input name="familyname" placeholder="Family Name" value={familyname} onChange={this.handleChange}/><span className="error-message">*</span></div>
      ...
      <div className='error-message'>{phonenumberError}</div>
        <div style={style}>
              <select  className="select" id="select" name="relation" required onChange={this.handleSelect}>
      ...
      <div style={style}><input name="email" placeholder="email" value={email} onChange={this.handleChange}/><span className="error-message">*</span></div>
      <div className='error-message'>{emailError}</div>
      <button type="button" value="Add" disabled={!isValid || defaultError} onClick={this.handleAdd}>Add </button>
    </form>
    </div>
    <div className='container'>
        {this.state.items.map((item) => {
          ...
          return (
            <section key={id} className='card'>
              <div><FaEdit /> <FaTrash onClick={() =>this.submit(id)}/></div>
              <h4>{name} {familyname}</h4>
              <p>{phonenumber}</p>
              <p>{options}</p>
              <p>{email}</p>
            </section>
